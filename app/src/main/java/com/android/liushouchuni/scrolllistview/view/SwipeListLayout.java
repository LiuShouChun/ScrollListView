package com.android.liushouchuni.scrolllistview.view;
...
public class SwipeListLayout extends FrameLayout {
	...
	protected void onFinishInflate() {
		...
		hiddenView = getChildAt(0); // 得到隐藏按钮的linearlayout 也就是置顶和删除按钮
	...
	Callback callback = new Callback() {
		//返回true表示捕获当前touch到的view，可以根据第一个参数child决定捕获哪些view。child是当前触摸到的view。
		@Override
		...
		public int clampViewPositionHorizontal(View child, int left, int dx) {
          ...
          //拽子View在相应方向上可以被拖动的最远距离，默认为0  也就是本项目的置顶 删除按钮的布局
		@Override
		...
		/**
		 @param changedView changedView当captureView的位置发生变化时的回调
